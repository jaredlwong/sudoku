...
function string_to_puzzle(input: string): number[] {
    return input.split('').map((c) => c === '.' ? 0 : parseInt(c, 10));
}

function puzzle_to_string(puzzle: number[]): string {
    return puzzle.map((e) => e === 0 ? '.' : e.toString()).join('');
}

function is_valid_row(row: number[]): boolean {
...
function is_valid(grid: number[]): boolean {
    for (let r = 0; r < 9; r++) {
...
function next_open(grid: number[]): number {
    for (let i = 0; i < 81; i++) {
        if (grid[i] === 0) {
            return i;
        }
...
function solve(grid: number[]): number[] | undefined {
    if (!is_valid(grid)) {
        return;
    }
